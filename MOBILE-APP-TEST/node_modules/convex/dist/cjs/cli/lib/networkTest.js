...
async function checkDns(ctx, url) {
  try {
    ...
    (0, import_log.logMessage)(
      `${import_chalk.chalkStderr.green(`\u2714`)} OK: DNS lookup => ${result.address}:${ipFamilyNames[result.family]} (${(0, import_utils.formatDuration)(result.duration)})`
    );
  } catch (e) {
    return ctx.crash({
      exitCode: 1,
...
async function checkTcpHostPort(ctx, host, port, ipFamilyOpt) {
  ...
  try {
    ...
    (0, import_log.logMessage)(
      `${import_chalk.chalkStderr.green(`\u2714`)} OK: ${tcpString} connect (${(0, import_utils.formatDuration)(
        duration
      )})`
    );
  } catch (e) {
...
async function checkHttpOnce(ctx, name, url, allowRedirects) {
  ...
  (0, import_log.logMessage)(
    `${import_chalk.chalkStderr.green(`\u2714`)} OK: ${name} check (${(0, import_utils.formatDuration)(duration)})`
  );
}
async function checkWs(ctx, { url, adminKey }) {
  ...
  if (cloudUrl === null) {
    ...
    (0, import_log.logMessage)(
      `${import_chalk.chalkStderr.green(`\u2714`)} OK: WebSocket connection established.`
    );
  }
}
async function checkEcho(ctx, url, size) {
  try {
      ...
      onError: (err) => {
        (0, import_log.logFailure)(
          import_chalk.chalkStderr.red(
            `FAIL: echo ${(0, import_utils.formatSize)(size)} (${err}), retrying...`
          )
        );
      }
    });
    ...
    (0, import_log.logMessage)(
      `${import_chalk.chalkStderr.green(`\u2714`)} OK: echo ${(0, import_utils.formatSize)(size)} (${(0, import_utils.formatDuration)(
        duration
