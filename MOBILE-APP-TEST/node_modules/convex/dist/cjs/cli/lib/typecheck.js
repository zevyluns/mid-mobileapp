...
__export(typecheck_exports, {
  resolveTypescriptCompiler: () => resolveTypescriptCompiler,
...
async function resolveTypescriptCompiler(ctx, cliOption) {
...
async function typeCheckFunctionsInMode(ctx, typeCheckMode, functionsDir) {
  ...
  const typecheckStart = import_perf_hooks.performance.now();
  ...
  maybeLogSlowTypecheckSuggestion(
    ctx,
    import_perf_hooks.performance.now() - typecheckStart,
    typescriptCompiler
  );
}
async function typeCheckFunctions(ctx, typescriptCompiler, functionsDir, handleResult) {
...
async function runTsc(ctx, typescriptCompiler, tscArgs, handleResult) {
  const tscPath = typescriptCompiler === "tsgo" ? import_path.default.join(
    "node_modules",
    "@typescript",
    "native-preview",
    "bin",
    "tsgo.js"
  ) : import_path.default.join("node_modules", "typescript", "bin", "tsc");
...
function maybeLogSlowTypecheckSuggestion(ctx, durationMs, typescriptCompiler) {
  if (!(ctx instanceof import_watch.WatchContext)) {
    return;
  }
  if (!ctx.isFirstPush) {
    return;
  ...
  const formattedDuration = (0, import_utils.formatDuration)(durationMs);
