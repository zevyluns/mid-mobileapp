...
async function findLatestVersionWithBinary(ctx, requireSuccess) {
  async function maybeCrash(...args) {
    ...
    if (args[0].printedMessage) {
      ...
      (0, import_log.logError)("Error fetching latest version");
    }
    return null;
  }
  (0, import_log.logVerbose)("Fetching latest backend version from version API");
  try {
    const response = await fetch(
      "https://version.convex.dev/v1/local_backend_version",
      {
        headers: { "Convex-Client": `npm-cli-${import_version.version}` }
      }
    );
    if (!response.ok) {
    ...
    (0, import_log.logVerbose)(`Latest backend version is ${data.version}`);
    return data.version;
  } catch (e) {
    return maybeCrash({
      exitCode: 1,
