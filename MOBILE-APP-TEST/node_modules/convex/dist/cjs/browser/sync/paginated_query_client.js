...
var paginated_query_client_exports = {};
__export(paginated_query_client_exports, {
  PaginatedQueryClient: () => PaginatedQueryClient
});
module.exports = __toCommonJS(paginated_query_client_exports);
...
class PaginatedQueryClient {
  constructor(client, onTransition) {
    ...
    this.lastTransitionTs = import_long.Long.fromNumber(0);
  ...
  subscribe(name, args, options) {
    const canonicalizedUdfPath = (0, import_udf_path_utils.canonicalizeUdfPath)(name);
    const token = (0, import_udf_path_utils.serializePaginatedPathAndArgs)(
  ...
  localQueryResultByToken(token) {
    ...
    for (const pageToken of activePages) {
      ...
      const paginationResult = (0, import_pagination.asPaginationResult)(result);
  ...
  loadMoreOfPaginatedQuery(token, numItems) {
    ...
    const paginationResult = (0, import_pagination.asPaginationResult)(lastPageResult);
  ...
  processPaginatedQuerySplits(changed, getResult) {
    for (const paginatedQueryToken of changed) {
      ...
      for (const pageKey of pageKeys) {
        ...
        const result = (0, import_pagination.asPaginationResult)(pageResult);
