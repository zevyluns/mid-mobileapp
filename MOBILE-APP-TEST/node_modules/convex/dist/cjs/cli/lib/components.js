...
__export(components_exports, {
  hash: () => hash,
  partitionModulesByChanges: () => partitionModulesByChanges,
...
async function runCodegen(ctx, deploymentSelection, options) {
  ...
  if (options.init) {
    await (0, import_codegen.doInitConvexFolder)(ctx, functionsDirectoryPath, {
...
function hash(bundle) {
  return (0, import_crypto.createHash)("sha256").update(bundle.source).update(bundle.sourceMap || "").digest("hex");
}
function isModuleTheSame(newBundle, oldBundleHash) {
  return newBundle.environment === oldBundleHash.environment && hash(newBundle) === oldBundleHash.hash;
}
function partitionModulesByChanges(functions, remoteHashesByPath) {
  const unchangedModuleHashes = functions.filter((newBundle) => {
...
async function getUnchangedModuleHashesFromServer(ctx, appImplementation, options) {
...
async function startComponentsPushAndCodegen(ctx, parentSpan, projectConfig, configPath, options) {
  ...
  const { unchangedModuleHashes, changedModules } = options.pushAllModules ? {
    ...
    "getUnchangedModuleHashesFromServer",
