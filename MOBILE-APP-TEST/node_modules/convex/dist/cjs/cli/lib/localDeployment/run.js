...
__export(run_exports, {
  ...
  shouldUseLocalDeployment: () => shouldUseLocalDeployment,
  withRunningBackend: () => withRunningBackend
});
...
async function isLocalBackendRunning(url, deploymentName) {
  return "running" === (await fetchLocalBackendStatus({ url, deploymentName })).kind;
}
function shouldUseLocalDeployment(deploymentType) {
  return deploymentType === "local" || deploymentType === "anonymous";
}
async function withRunningBackend({
  ctx,
  deployment,
  action
}) {
...
async function startEphemeralLocalBackend(ctx, args) {
  ...
  const config = (0, import_filePaths.loadDeploymentConfig)(ctx, deploymentKind, args.deploymentName);
  if (config === null) {
    return ctx.crash({
      exitCode: 1,
      errorType: "fatal",
...
Run \`npx convex dev\` first.`
  ...
  const instanceSecret = config.instanceSecret ?? import_utils.LOCAL_BACKEND_INSTANCE_SECRET;
