...
__export(dataModel_exports, {
  dynamicDataModelDTS: () => dynamicDataModelDTS,
  dynamicDataModelTS: () => dynamicDataModelTS,
  noSchemaDataModelDTS: () => noSchemaDataModelDTS,
  noSchemaDataModelTS: () => noSchemaDataModelTS,
  staticDataModelDTS: () => staticDataModelDTS,
  staticDataModelTS: () => staticDataModelTS
});
...
function dynamicDataModelTS() {
  return dynamicDataModelContent;
}
async function staticDataModelImpl(ctx, startPush, rootComponent, componentDirectory, useTypeScript) {
...
async function staticDataModelDTS(ctx, startPush, rootComponent, componentDirectory) {
...
async function staticDataModelTS(ctx, startPush, rootComponent, componentDirectory) {
  return staticDataModelImpl(
    ctx,
    startPush,
    rootComponent,
...
async function* codegenDataModel(ctx, schema) {
  ...
  tables.sort((a, b) => (0, import_common.compareStrings)(a.tableName, b.tableName));
...
async function* codegenTable(ctx, table) {
  ...
  yield `{`;
  yield `  document: ${(0, import_validator_helpers.validatorToType)(documentType, true)},`;
  const fieldPaths = /* @__PURE__ */ new Set();
  for (const fieldPath of extractFieldPaths(documentType)) {
    fieldPaths.add(fieldPath.join("."));
  }
  yield `  fieldPaths: ${stringLiteralUnionType(Array.from(fieldPaths).sort(import_common.compareStrings))},`;
  yield `  indexes: {`;
  const systemIndexes = {
    by_id: ["_id"],
