  ...
  try {
    ...
    if (errorMessage.includes("Could not find function")) {
      ...
      const functionNames = functions.filter(
        ...
        return `\u2022 ${import_chalk.chalkStderr.gray(`${path2}:`)}${name}`;
      });
...
${import_chalk.chalkStderr.red(errorMessage)}
...
${import_chalk.chalkStderr.red(err.toString().trim())}`
    });
  }
