...
async function exportFromDeployment(ctx, options) {
  ...
  (0, import_log.showSpinner)(`Downloading snapshot export to ${import_chalk.chalkStderr.bold(inputPath)}`);
  ...
  (0, import_log.stopSpinner)();
  (0, import_log.logFinishedStep)(
    `Downloaded snapshot export to ${import_chalk.chalkStderr.bold(filePath)}`
  );
}
  ...
  if (ctx.fs.exists(inputPath)) {
    ...
    if (st.isDirectory()) {
      ...
      return await ctx.crash({
        ...
        printedMessage: `Error: Path ${import_chalk.chalkStderr.bold(inputPath)} already exists.`
      });
    }
  } else {
    filePath = inputPath;
  }
  (0, import_log.changeSpinner)(`Downloading snapshot export to ${import_chalk.chalkStderr.bold(filePath)}`);
  try {
    ...
    (0, import_log.logError)(import_chalk.chalkStderr.red(e));
    return await ctx.crash({
      exitCode: 1,
      errorType: "fatal",
      printedMessage: `Exporting data failed: ${import_chalk.chalkStderr.red(e)}`
    });
  }
  return { filePath };
}
