...
const workosTeamStatus = new Command("status").summary("Status of associated WorkOS team and environment").addDeploymentSelectionOptions(actionDescription("Check WorkOS status for")).action(async (_options, cmd) => {
...
const workosInvite = new Command("invite").summary("Invite yourself to the WorkOS team").description(
...
const workosProjectEnvList = new Command("list-project-environments").summary("List WorkOS environments for current project").description(
  ...
  try {
    ...
    if (environments.length === 0) {
      ...
      for (const env of environments) {
        const prodLabel = env.isProduction ? chalkStderr.yellow(" (production)") : "";
        logMessage(
...
const workosProjectEnvCreate = new Command("create-project-environment").summary("Create a new WorkOS environment for the project").description(
...
const workosProjectEnvDelete = new Command("delete-project-environment").summary("Delete a WorkOS environment from the project").description(
...
const workos = new Command("workos").summary("WorkOS integration commands").description("Manage WorkOS team provisioning and environment setup").addCommand(workosProvisionEnvironment).addCommand(workosTeamStatus).addCommand(workosProvisionTeam).addCommand(workosDisconnectTeam).addCommand(workosInvite).addCommand(workosProjectEnvList).addCommand(workosProjectEnvCreate).addCommand(workosProjectEnvDelete);
