...
async function importIntoDeployment(ctx, filePath, options) {
  if (!ctx.fs.exists(filePath)) {
    return await ctx.crash({
      ...
      printedMessage: `Error: Path ${import_chalk.chalkStderr.bold(filePath)} does not exist.`
    });
  }
  ...
  const tableNotice = tableName ? ` to table "${import_chalk.chalkStderr.bold(tableName)}"` : "";
  const onFailure = async () => {
    (0, import_log.logFailure)(
      `Importing data from "${import_chalk.chalkStderr.bold(
        filePath
    ...
    switch (snapshotImportState.state) {
      ...
      case "failed":
        return await ctx.crash({
          exitCode: 1,
          errorType: "fatal",
          printedMessage: `Importing data from "${import_chalk.chalkStderr.bold(
            filePath
          )}"${tableNotice}${options.deploymentNotice} failed

${import_chalk.chalkStderr.red(snapshotImportState.error_message)}`
        });
