...
async function selectRegionOrUseDefault(ctx, selectedTeam, deploymentType) {
  const noDefaultRegionMessage = import_chalk.chalkStderr.gray(
    `Tip: you can configure a default region for your team at ${import_chalk.chalkStderr.underline(`https://dashboard.convex.dev/t/${selectedTeam.slug}/settings`)}`
  );
  ...
  const selectedRegionName = selectedTeam.defaultRegion ?? await selectRegion(ctx, selectedTeam.id, deploymentType);
  if (!selectedTeam.defaultRegion) {
    (0, import_log.logMessage)(noDefaultRegionMessage);
  }
  return selectedRegionName;
}
async function selectRegion(ctx, teamId, deploymentType) {
...
async function bigBrainFetch(ctx) {
  ...
  return (resource, options) => {
    ...
    const headers = {
      ...bigBrainHeaders,
      ...optionsHeaders ?? (resource instanceof Request ? Object.fromEntries(resource.headers.entries()) : {})
    };
    const opts = {
      retries: MAX_RETRIES,
      retryDelay,
